...
Server::Server(int port){
    ...
    pthread_t thread1;
    int rc = pthread_create(&thread1, NULL, exitThread , NULL);
    if (rc) {
        cout << "Error: unable to create exitThread, " << rc << endl;
        exit(-1);
    }

    //make the main thread to get connection with clients
    pthread_t thread2;
    rc = pthread_create(&thread2, NULL, exitThread , NULL);
    if (rc) {
        cout << "Error: unable to create exitThread, " << rc << endl;
        exit(-1);
    }
